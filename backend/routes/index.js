...
router.post("/signup", async (req, res) => {
  ...
  if (emailcon) {
    ...
    bcrypt.hash(password, 10, async function (err, hash) {
      ...
      var token = jwt.sign({ userID: user._id, email: user.email }, secret, {expiresIn:'0.5h'});
...
router.post("/getnotes", async (req, res) => {
  ...
  if (notes.length > 0) {
    res.json({ success: true, message: "Notes found", notes: notes });
  } else {
    res.status(400).json({ message: "No notes found", success: false });
  }
});
...
router.post("/updatenote", async (req, res) => {
  ...
  let note = await notesModel.findByIdAndUpdate(noteId, {
    title,
    description,
    content,
    isImportant,
